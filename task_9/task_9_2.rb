def read_movies(movies_file)
  movies_hash = {}

  movies_file.each_line do |line|
    rators_hash = {}
    movie_name, name_rating = line.split(" : ")
    rates = name_rating.split

    rates.each do |rating|
      name_of_raters, rating = rating.split("-")
      rators_hash[name_of_raters] = rating.to_f
    end
    movies_hash[movie_name] = rators_hash
  end
  movies_file.close
  return movies_hash
end
def write_movies(movies)
  count = 0
  movies_file = File.open("movies.txt", "w")
  movies.each do |line|
    rating = line[1].to_a
    movies_file.write("#{line[0]} : ")
    rating.each do |rate|
      movies_file.write("#{rate[0]}-#{rate[1]} ")
    end
      movies_file.write("\n")
  end
  movies_file.close
...
movies_file = File.open("movies.txt")
...
loop do
  ...
  when "add"
    ...
    if new_movie.include?(":") || new_movie.include?("-")
        p "Wrong charactor! ('-' or ':')"
    else
